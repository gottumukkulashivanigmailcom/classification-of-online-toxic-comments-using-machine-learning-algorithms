...
def preprocess():
    ...
    for i in range(len(dataset)):
        msg = dataset.loc[i, 'comment_text']
        toxic = dataset.loc[i, 'toxic']
        severe_toxic = dataset.loc[i, 'severe_toxic']
        obscene = dataset.loc[i, 'obscene']
        threat = dataset.loc[i, 'threat']
        insult = dataset.loc[i, 'insult']
        identity_hate = dataset.loc[i, 'identity_hate']
        msg = str(msg)
        msg = msg.strip().lower()
        Y1.append(int(toxic))
